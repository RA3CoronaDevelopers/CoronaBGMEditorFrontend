...
class Connection {
  ...
  send(message: InputMessage) {
    return this.ws.sendPacked(message)
  }
  sendRequest(message: InputMessage) {
    return this.ws.sendRequest(message)
  }
  ...
  addMessageHandler<TKey extends string, TValue extends string>(
    ...
    handler: (
      message: Extract<OutputMessage, Record<TKey, TValue>>
  ...
  useMessageHandler<
    TKey extends keyof OutputMessage,
    TValue extends OutputMessage[TKey]
  >(
    key: TKey,
    value: TValue,
    handler: (
